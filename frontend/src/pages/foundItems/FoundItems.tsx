  ...
  return (
    <div className="min-h-screen bg-gray-950 pb-16">
            ...
            <form className="mb-6" onSubmit={handleSearch}>
              ...
              <div className="relative">
                ...
                <input
                  type="search"
                  id="default-search"
                  className="block w-full p-4 ps-12 text-sm text-white bg-gray-800/50 border border-red-900/40 hover:border-red-800/60 rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-500/70 focus:border-yellow-500/60 placeholder-gray-400 transition-all duration-200"
                ...
                <button
                  type="submit"
                  className="text-white absolute end-3 bottom-2.5 bg-gradient-to-br from-red-700 to-red-900 hover:from-red-600 hover:to-red-800 focus:outline-none focus:ring-2 focus:ring-yellow-500/70 font-medium rounded-lg text-sm px-6 py-2 transition-all duration-200 shadow-[0_2px_8px_rgba(128,0,0,0.35)] hover:shadow-[0_4px_14px_rgba(128,0,0,0.5)]"
                >
                  Search
                </button>
              </div>
        ...
        {foundItems?.data?.length === 0 ? (
          <div className="text-center py-16">
            ...
            {searchTerm && (
              <button
                onClick={clearSearch}
                className="inline-flex items-center px-6 py-3 text-sm font-medium text-white bg-gradient-to-r from-red-800 to-red-900 hover:from-red-900 hover:to-red-950 rounded-lg shadow-lg hover:shadow-xl transition-all duration-200"
              >
          ...
          <div className="grid gap-6 mx-auto grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {foundItems?.data?.map((foundItem: any) => {
              return (
                <div
                  key={foundItem.id}
                  className="group relative bg-gray-800/60 backdrop-blur-sm rounded-xl overflow-hidden border-t-2 border-t-yellow-600/60 border border-gray-700/60 shadow-sm hover:shadow-[0_4px_14px_rgba(128,0,0,0.25)] hover:scale-[1.02] transition-all duration-200 max-w-sm"
                >
      ...
      <PaginationNav
        ...
        totalItems={foundItems?.meta?.total || 0}
